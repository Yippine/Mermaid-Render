...
export class CytoscapeEngine implements RenderEngine {
  ...
  // 數據轉換
  private async convertToCytoscapeFormat(
    data: GraphData
  ...
  async showSequence(sequence: SequenceStep[]): Promise<void> {
    ...
    this.currentSequence = new SequenceController(
      this.cy,
...
export class ELKEngine implements RenderEngine {
  ...
  private createSVGNode(node: ELKNode): SVGGElement {
    ...
    if (node.labels && node.labels.length > 0) {
      ...
      text.setAttribute(
        'y',
        ((node.y || 0) + (node.height || 0) / 2).toString()
      )
...
export class LayoutAnalyzer {
  ...
  private detectHierarchy(data: GraphData): boolean {
    ...
    return (
      roots.length > 0 &&
      leaves.length > 0 &&
      roots.length < data.nodes.length * 0.3
    )
  }
    ...
    return {
      ...
      progress:
        this.timeline.length > 0
          ? (this.currentStep + 1) / this.timeline.length
          : 0,
...
export class PerformanceOptimizer {
  ...
  private cullElementsOutsideViewport(viewport: Viewport): void {
    ...
    const hiddenElements = this.renderEngine
      .getAllElements()
