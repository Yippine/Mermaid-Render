...
export interface ExportService {
  ...
  // 批量匯出
  exportBatch(requests: ExportRequest[]): Promise<BatchExportResult>
...
export class ExportStrategySelector {
  ...
  private shouldUseServerExport(
    ...
    // 超大圖表或複雜度過高使用伺服器處理
    return (
      memoryRequired > 500 * 1024 * 1024 || // 500MB
  ...
  private shouldUseChunkedExport(
    ...
    return (
      dimensions.width > maxCanvasSize.width ||
...
export class ChunkedExportStrategy extends ExportStrategy {
  ...
  private async mergeChunks(
    ...
    const mergeCanvas = new OffscreenCanvas(
    ...
    for (const result of chunkResults) {
      ...
      // 複製到主 Canvas
      mergeCtx.drawImage(tempCanvas, result.bounds.x, result.bounds.y)
    }
  ...
  private filterGraphDataByBounds(
    ...
    const visibleNodes = graphData.nodes.filter(node => {
      ...
      return (
        this.isPositionInBounds(node.position, bounds) ||
...
export class ProgressTracker {
  ...
  getProgress(): ExportProgress {
    ...
    const progressRatio =
      this.totalSteps > 0 ? this.completedSteps / this.totalSteps : 0
    const estimatedTotalTime =
      progressRatio > 0 ? elapsedTime / progressRatio : 0
